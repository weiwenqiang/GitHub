...
public class MainActivity extends Activity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.list:
                getFragmentManager().beginTransaction().setCustomAnimations(R.animator.translate_right_in,
                        R.animator.translate_left_out).replace(R.id.fragment_container, listFragment).commit();
                break;
            case R.id.grid:
                getFragmentManager().beginTransaction().setCustomAnimations(R.animator.translate_right_in,
                        R.animator.translate_left_out).replace(R.id.fragment_container, gridFragment).commit();
                break;
            case R.id.staggered_grid:
                getFragmentManager().beginTransaction().setCustomAnimations(R.animator.translate_right_in,
                        R.animator.translate_left_out).replace(R.id.fragment_container, staggeredGridFragment).commit();
                break;
        }
    }
}
