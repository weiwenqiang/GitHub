package com.hpw.myapp.widget.imageselector.view;
...
import com.hpw.myapp.widget.imageselector.utils.GridSpacingItemDecoration;
import com.hpw.myapp.widget.imageselector.utils.LocalMediaLoader;
        ...
        imageAdapter.setOnImageSelectChangedListener(new ImageListAdapter.OnImageSelectChangedListener() {
            ...
            public void onPictureClick(LocalMedia media, int position) {
                if (enablePreview) {
                    ...
                    if (imageAdapter.getImages().size() > 530) {
                        for (int i = 0; i < 530; i++) {
                            localMedias.add(imageAdapter.getImages().get(i));
                        }
                        startPreview(localMedias, position);
                    } else {
